...
// Read from Couchdb database
try {
	$data = $client->group_level ( 1 )->getView ( 'count', 'lang' ); //Read view - lang
} catch ( Exception $e ) {
	if ($e->code () == 404) {
...
usort ( $lang, function ($a, $b) { // Sort the array using a user defined function
	return $a->value > $b->value ? - 1 : 1; // Compare the scores
} );

$arr2 = json_encode ($lang);
